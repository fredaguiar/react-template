...
/**
 * enterprise Authentication setup will initialize permit, intialize passport authentication to the enterprise IDP, and will configure express with
...
function setupAuthRoutes(express) {
  ...
  express.get(settings.logoutPath, (req, res) => {
    ...
    res.send('Logged out'); // TODO: should make a enterprise style logout page for this
  });
  express.post(
  ...
  express.get(settings.failPath, function fn(req, res) {
    res.status(401).send('Login failed'); // TODO: should create a enterprise style failure page for this
  });
