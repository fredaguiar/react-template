...
const enterpriseAuth = require('./auth');
...
if (enabledSSO && !development) {
  enterpriseAuth.authSetup(server, config.sessionSecret, config.sso, config.permit);
}
...
if (enabledSSO && !development) {
  server.get('/authorized', (req, res) => {
    enterpriseAuth.isAuthorized(req.user, 'resource', 'action').then(authorized => {
      if (authorized) {
